...
export class GPUService extends Service {
    ...
    routes:Routes = {
        addFunc:(fnString:string)=>{
            this.gpu.addFunction(parseFunctionFromText(fnString))
        },
        addKernel:(name:string,fnString:string,options?:any)=>{
            this.gpu.addKernel(name,parseFunctionFromText(fnString),options);
        },
        callKernel:(name:string,...args)=>{
