...
export class Router {
      ...
      return new Promise(resolve => {
          ...
          if (service.subscribe){
            service.subscribe(async (o:MessageObject, type:MessageType) => {
                ...
                console.log('NOTIFY', name, client, o, type)
          ...
          if (onlySubscribe) resolve(service)
            ...
            this.SERVICES[name] = service 
              ...
              // TODO: Don't Resolve Unless Matching Service at Endpoint...
              this.SERVICES[name].status = true
  ...
  handleLocalRoute = async (o:MessageObject, endpoint?: Endpoint, route?: string) =>{
    ...
    if (!o.block) {
      ...
      console.log('try', route, o.route, this.ROUTES)
      if (route?.post instanceof Process) return route.post.run(this, o.message, o.id) // TODO: Enable non-post
    }
  }
  ...
  sendMsg(user:string|UserObject='',message='',data=undefined) {
      ...
      return new Promise(async resolve => {
        ...
        Promise.all(possibilities.map(async possibleRoute => {
          ...
          if (routeInfo) {
            ...
            try {
              let res;
              const postProcess = routeInfo?.post instanceof Process
    ...
    async checkRoutes(event) {
        ...
        if (route){
              if (event.data.message && route.post instanceof Process) return await route.post.run(this, event.data.message, event.data.origin);
              else return
        } else return false
    }
}
