...
export class ParticleDynamics {
  ...
  checkParticleBounds = (particle) => {
      ...
      // Give the particle some bounce
      if ((particle.positiony + particle.particleSize) > particle.boundingBox.bot) {
  ...
  boidsTimestepFunc = (group,timeStep) => {
      ...
      if(success) {
          ...
          group.particles.forEach((p,i) => {
              p.boid.attractor.x = Math.sin(anchorTick)*Math.sin(anchorTick)*p.boundingBox.right*0.3+p.boundingBox.right*0.2;
              p.boid.attractor.y = p.boundingBox.bot*0.3;
