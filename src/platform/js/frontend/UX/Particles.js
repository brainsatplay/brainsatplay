...
export class ParticleDynamics {
  constructor(
    ...
    this.prototype = {
      ...
      velocity:{x:0,y:0,z:0,lastx:0,lasty:0,lastz:0},
      ...
      lifeTime: Infinity, //Number of seconds before the particle despawns
      boundingBox = {left:0,right:1,bot:1,top:0}, //bounding box, 1 = max height/width of render window
      boid:{
        boundingBox = {left:0,right:1,bot:1,top:0}, //bounding box, 1 = max height/width of render window
        cohesion:0.001,
        separation:1,
        alignment:0.5,
        useSwirl:true,
        swirl:{x:0.5,y:0.5,z:0.5,mul:0.0001},
        useAttractor:true,
  ...
  distance3D(a,b) //assumes you're passing two Array(3) i.e. [x,y,z]
  {
    return Math.sqrt((b[0]*b[0]-a[0]*a[0]) + (b[1]*b[1]-a[1]*a[1]) + (b[2]*b[2]-a[2]*a[2]));
  }

  normalize3D(vec3 = []) {
    var normal = Math.sqrt(vec3[0]*vec3[0]+vec3[1]*vec3[1]+vec3[2]*vec3[2]);
  ...
  defaultBoidGroupRule = (particle,rule) => {
    ...
    if(this.canvas) {
      ...
      particle.startingX = Math.random()*w;
      particle.startingY = Math.random()*h;
  ...
  checkParticleBounds = (particles,i) => {
     
    if((particles[i].velocity.x > particles[i].maxSpeed) || (particles[i].velocity.y > particles[i].maxSpeed) || (particles[i].velocity.z > particles[i].maxSpeed) || (particles[i].velocity.x < -particles[i].maxSpeed) || (particles[i].velocity.y < -particles[i].maxSpeed) || (particles[i].velocity.z < -particles[i].maxSpeed)) {
      let normalized = this.normalize3D([particles[i].velocity.x,particles[i].velocity.y,particles[i].velocity.z]);
      particles[i].velocity.x = normalized[0]*particles[i].maxSpeed;
      particles[i].velocity.y = normalized[1]*particles[i].maxSpeed;
    }
    
    // Give the particle some bounce
    if ((particles[i].position.y + particles[i].particleSize) > particles[i].boundingBox.bot) {
      particles[i].velocity.y *= particles[i].yBounce;
      particles[i].velocity.x *= -particles[i].xBounce;
      particles[i].position.y = particles[i].boundingBox.bot - particles[i].particleSize;
    }

    // Give the particle some bounce
    if ((particles[i].y - particles[i].particleSize) < particles[i].boundingBox.top) {
      particles[i].velocity.y *= particles[i].yBounce;
      particles[i].velocity.x *= -particles[i].xBounce;
      particles[i].position.y = particles[i].boundingBox.top + particles[i].particleSize;
    }

    // Determine whether to bounce the particle off a wall
    if (particles[i].position.x - (particles[i].particleSize) <= particles[i].boundingBox.left) {
      particles[i].velocity.x *= particles[i].xBounce;
      particles[i].position.x = particles[i].boundingBox.left + (particles[i].particleSize);
    }

    if (particles[i].x + (particles[i].particleSize) >= particles[i].boundingBox.right) {
      particles[i].velocity.x *= particles[i].xBounce;
      particles[i].position.x = particles[i].boundingBox.right - particles[i].particleSize;
    }
  }


  defautTimestepFunc = (group,timeStep)=>{ //what happens on each time step?
    ...
    group.particles.forEach((p,i) => {
      ...
      this.checkParticleBounds(particles,i);
    ...
    calcBoids = (particles=[]) => {
      ...
      for(var i = 0; i < particles.length; i++) {
        ...
        for(let j = 0; j < particles.length; j++) {
          ...
          separationVec[0] = separationVec[0] + (p0.position.x-pr.position.x)*(1/disttemp);
          separationVec[1] = separationVec[1] + p0.position.y-pr.position.y*(1/disttemp); 
          separationVec[2] = separationVec[2] + (p0.position.z-pr.position.z + 1)*(1/disttemp);
          if((separationVec[0] == Infinity) || (separationVec[0] == -Infinity) || (separationVec[0] > 3) || (separationVec[0] < -3) || (separationVec[1] == Infinity) || (separationVec[1] == -Infinity) || (separationVec[1] > 3) || (separationVec[1] < -3) || (separationVec[2] == Infinity) || (separationVec[2] == -Infinity) || (separationVec[2] > 3) || (separationVec[2] < -3) ) {
            separationVec[0] = Math.random()*4-2;
            separationVec[1] = Math.random()*4-2;
            separationVec[2] = Math.random()*4-2; //Special case for when particles overlap and cause infinities
            //console.log("Infinity!")
          }
          //console.log(separationVec);
  ...
  boidsTimestepFunc = (group,timeStep) => {
    ...
    if(success) {
      ...
      group.particles.forEach((p) => {
        p.boid.anchor.x = Math.sin(anchorTick)*Math.sin(anchorTick)*p.boundingBox.right*0.3+p.boundingBox.right*0.2;
        p.boid.anchor.y = p.boundingBox.bot*0.3;
        p.boid.anchor.z = 0;
        ...
        this.checkParticleBounds(particles,i,timeStep);
  ...
  loop = (lastFrame=performance.now()*0.001) => {
    ...
    setTimeout(()=>{requestAnimationFrame(()=>{this.loop(currFrame)})},15);
  }
