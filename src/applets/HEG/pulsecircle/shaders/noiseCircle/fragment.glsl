...
void main()
{
    ...
    for (int i = 0; i < 5; i++){
        ...
        outColor += vec4(colors[i]*alpha*vec3(circle(mouse,uv,outerDiameter)), alpha); // Outer Diameter
        outColor -= vec4(colors[i]*alpha*vec3(circle(mouse,uv,innerDiameter)), alpha); // Inner Diameter
    }
