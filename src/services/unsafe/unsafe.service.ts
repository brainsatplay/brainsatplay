...
class UnsafeService extends Service {
    ...
    routes = [
        { //add a local function, can implement whole algorithm pipelines on-the-fly
          route: 'createRoute', post: async (self, router, args) => { //arg0 = name, arg1 = function string (arrow or normal)
        ...
        { //set locally accessible values, just make sure not to overwrite the defaults in the callbackManager
          route: 'setValues', post: (self, router, args) => {
        ...
        { //append array values
          route: 'appendValues', post: (self, router, args) => {
            if (typeof args === 'object') {
              Object.keys(args).forEach((key) => {
        ...
        { //for use with transfers
          route: 'setValuesFromArrayBuffers', post: (self, router, args) => {
            if (typeof args === 'object') {
              Object.keys(args).forEach((key) => { 
        ...
        { //for use with transfers
          route: 'appendValuesFromArrayBuffers', post: (self, router, args) => {
            if (typeof args === 'object') {
              Object.keys(args).forEach((key) => {
        ...
        { //parses a stringified class prototype (class x{}.toString()) containing function methods for use on the worker
          route: 'transferClassObject', post: (self, router, args) => {
            if (typeof args === 'object') {
              Object.keys(args).forEach((key) => {
